...
int connect_source_buffers(Sourceparams_t * sourceparams)
{
  ...
  if (TESTPATTERN ==  sourceparams->source)
    ...
    switch(sourceparams->iomethod)
        {
        case IO_METHOD_READ:
          retval = 0;//allocate_capture_buffer(sourceparams);
          break;
        case IO_METHOD_MMAP:
			     ...
			     Videocapabilities_t * capabilities)
{
  ...
  //ali try_reset_crop_scale(sourceparams);
		  ...
		  Videocapabilities_t * capabilities)
{
  ...
  switch (sourceparams->iomethod)
    {
    case IO_METHOD_READ:
      retval = 0;//init_read_io(p_sourceparams, p_capabilities);
      break;
    case IO_METHOD_MMAP:
      retval = init_mmap_io(sourceparams, capabilities);
      break;
    case IO_METHOD_USERPTR:
      retval = 0;//init_userptr_io(p_sourceparams, p_capabilities);
      break;
    default:
...
int mmap_io_buffers(Sourceparams_t * sourceparams)
{
  ...
  status = 0;
  retval = 0;
fprintf(stderr, "requested bufcount=%d\n",sourceparams->buffercount);
...
int start_capture_device(Sourceparams_t * sourceparams)
{
  ...
  switch (sourceparams->iomethod)
    ...
    case IO_METHOD_MMAP:
        ...
        break;
	case IO_METHOD_USERPTR:
      retval = 0;//enqueue_userpointer_buffers(sourceparams);
      if (0 == retval)
	{
	  retval = start_streaming(sourceparams);
	} 
...
void * next_device_frame(Sourceparams_t * sourceparams, int * nbytesp)
{
  ...
  if (-1 == data_ready)
  ...
  else if (0 == data_ready)
    {
      ...
      datap = NULL;
      *nbytesp = 0;
      //fprintf(stderr, "Error:2 time out");
    }
  else
    {
	...
	case IO_METHOD_READ:
		  nbytes = 0;//read_video_frame(sourceparams->fd, &(sourceparams->captured));
		  if (0 < nbytes)
			{
	  ...
	  if (0 < nbytes)
	    {
	      datap = sourceparams->captured.start;
	      *nbytesp = nbytes;
	      //fprintf(stderr, "Error:5 harvest");
	    }
	  else
	    {
	      datap = NULL;
	...
	case IO_METHOD_USERPTR:
	  nbytes = 0;//harvest_userptr_device_buffer(sourceparams);
	  if (0 < nbytes)
	    {
	      datap = sourceparams->captured.start;
	      *nbytesp = nbytes;
...
int harvest_mmap_device_buffer(Sourceparams_t * sourceparams)
{
  ...
  if (-1 == status)
	...
	fprintf(stderr, "Error:7 can not DQBUF");
      switch(errno)
	{
	case EAGAIN:
	...
	default:
	  ...
	  break;
	  ...
	  else
		{
		  ...
		  if (-1 == status)
			{
				fprintf(stderr, "Error:8 can not QBUF");
	...
	//fprintf(stderr, "Error:1=%d",retval);
  return(retval);
}
